...
vec3 Scene::trace(const Ray &_ray, int _depth) {
    ...
    if (intersect(_ray, dist, ind)) {
        ...
        auto recursive_trace = [&](const vec3 &direction) {
            vec3 origin = point + direction * 0.01;
            Ray reflected_ray(origin, direction);
            return trace(reflected_ray, _depth + 1);
        };

        if (material.transparent) {
            vec3 color_reflection = recursive_trace(reflect(_ray.direction, normal)), color_refraction(0);
            float kr = fresnel(_ray.direction, normal, material.ior);
            if (kr < 1.f)
                color_refraction = recursive_trace(refract(_ray.direction, normal, material.ior));
            return mix(color_refraction, color_reflection, kr);
        } else {
            vec3 color(0), color_reflection(0);
            if (material.mirror != 1)
                color = lighting(point, normal, normalize(camera.eye_position() - point), material);
            if (material.mirror != 0)
                color_reflection = recursive_trace(reflect(_ray.direction, normal));
            return mix(color, color_reflection, material.mirror);
        }
...
bool Scene::intersect(const Ray &_ray, float &_distance, int &_ind) const {
    ...
    for (steps = 0; steps < max_ray_steps; steps++) {
        float d = std::abs(de(_ray(_distance), _ind));
        _distance += d;
